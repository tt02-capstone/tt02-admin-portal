...
import { getAllTelecomList } from "../../redux/telecomRedux";
    ...
    const column = [
        {
            ...
            key: 'telecom_id',
            sorter: (a, b) => a.telecom_id > b.telecom_id,
            ...getColumnSearchProps('telecom_id'),
        },
        {
            title: 'Name',
            dataIndex: 'name',
    ...
    useEffect(() => {
        if (fetchTelecomList) {
            const fetchData = async () => {
                const response = await getAllTelecomList();
                if (response.status) {
